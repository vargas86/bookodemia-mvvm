...
class ByNameViewController : UIViewController {
    
    var tableView : UITableView?
    var dataSource : CatalogObjectByTitle?
...
//    dataSource = catalog
    ...
    let T = IndexName(indexName: "T", books: [tresmujeres,talcomoeres])
    let K = IndexName(indexName: "K", books: [kamasutra])
    let C = IndexName(indexName: "C", books: [crimenycastigo])
    let M = IndexName(indexName: "M", books: [mobydick,madamebovary])
    let L = IndexName(indexName: "L", books: [senoradalloway])
    let D = IndexName(indexName: "D", books: [messi])
    let S = IndexName(indexName: "S", books: [cr7])
    let O = IndexName(indexName: "O", books: [pele])
            
    let catalogObjectByTitle = CatalogObjectByTitle(indexNames: [C,D,K,L,M,O,S,T], title: "Orden Alfabético")
    dataSource = catalogObjectByTitle
...
extension ByNameViewController : UITableViewDataSource{
    ///El numero de celdas por cada seccion
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return dataSource?.indexNames?[section].books?.count ?? 0
    }
    
    ///El tipo de celda que se mostrara
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let book = dataSource?.indexNames?[indexPath.section].books?[indexPath.row]
...
extension ByNameViewController : UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        label.text = dataSource?.indexNames?[section].indexName ?? ""
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let book = dataSource?.indexNames?[indexPath.section].books?[indexPath.row]
        let vc = DetailProductViewController(book: book!)
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        return dataSource?.indexNames?.count ?? 0
    }
